...
export async function getRecentEntries(filter: { days?: number; project?: string } = {}): Promise<JournalEntry[]> {
...
export async function searchEntries(query: string, limit: number = 20): Promise<JournalEntry[]> {
  return getDefaultJournal().searchCheckpoints(query, { limit, workspace: 'current' });
}
