...
class PlanStep(BaseModel):
    ...
    estimated_duration: Optional[str] = Field(
        default=None, 
        description="How long this step should take"
    )
...
class Plan(BaseModel):
    ...
    goals: list[str] = Field(
        default_factory=list,
        description="List of specific goals this plan should achieve"
    )
    
    success_criteria: list[str] = Field(
        default_factory=list,
        description="How to know when this plan is successfully completed"
    )
    
    # Steps and structure
    steps: list[PlanStep] = Field(
        default_factory=list,
        description="Ordered list of steps to complete this plan"
    )
    ...
    category: Optional[str] = Field(
        default=None,
        description="Category of plan (feature, bugfix, research, etc.)"
    )
    
    # Relationships
    related_todo_ids: list[str] = Field(
        default_factory=list,
        description="IDs of todos created from this plan"
    )
    
    related_checkpoint_ids: list[str] = Field(
        default_factory=list,
        description="IDs of checkpoints related to this plan"
    )
    
    # Metadata
    estimated_duration: Optional[str] = Field(
        default=None,
        description="Estimated total time to complete (e.g., '1w', '3d')"
    )
    ...
    completed_at: Optional[TZAwareDatetime] = Field(
        default=None,
        description="When this plan was completed"
    )
