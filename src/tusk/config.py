...
class TuskConfig(BaseModel):
    ...
    data_dir: Optional[Path] = Field(
        default=None,
        description="Custom data directory (overrides storage_mode if set)"
    )
    ...
    log_dir: Optional[Path] = Field(
        default=None,
        description="Directory for log files (auto-determined if not set)"
    )
    
    # Project settings
    auto_detect_project: bool = Field(
        default=True,
        description="Automatically detect current project from working directory"
    )
